...
creator.create("FitnessMax", base.Fitness, weights=(1000.0,-1.0,-0.0001))
...
def evalArtificialAnt(individual):
    ...
    ant.run(routine)
    #if(ant.max_moves>ant.moves):
    #  print(ant.moves)
    hgt = 1/(100.0 * individual.height) if (individual.height) else 0.
    return ant.eaten , ant.moves, individual.height #(ant.max_moves-ant.moves)/ant.max_moves + hgt
...
def tmin(t):
  v=list(zip(*t))
  if(len(v)>=2):
    return numpy.min(v[0]),numpy.min(v[1]),numpy.min(v[2])
  else:
    return 0,0,0
...
def tmax(t):
  v=list(zip(*t))
  if(len(v)>=2):
    return numpy.max(v[0]),numpy.max(v[1]),numpy.max(v[2])
  else:
    return 0,0,0
...
def main():
    ...
    stats.register("min", tmin)
    stats.register("max", tmax)
    
    #algorithms.eaSimple(pop, toolbox, 0.8, 0.1, 340, stats, halloffame=hof)
    algorithms.eaMuPlusLambda(pop, toolbox, mu=1000, lambda_=600, cxpb=0.6, mutpb=0.1, ngen=4000, stats=stats, halloffame=hof)
    
    hof.update(pop)
    print('Best individual : ', hof[0][0], hof[0].fitness)
    routine = gp.compile(hof[0], pset)
    ant.run(routine)
    print('number moves: ', ant.moves, hof[0].height)
